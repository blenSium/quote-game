...
class Friends {
  constructor() {}
  nextQuote(quoteArr, char) { //changing the quote randomly
    ...
    answersBtn[Math.floor(Math.random() * answersBtn.length)].innerText =
      this.index.character; // the right answer will be in a random button
...
const guessFriends = () => {
  axios
    ...
    .then((response) => {
      ...
      let characters = quotes.map((b) => b.character); // create an array of the characters names
      var mySet = new Set(characters); // array without duplicates
      characters = [...mySet];
...
guessFriends();  // calling the function
